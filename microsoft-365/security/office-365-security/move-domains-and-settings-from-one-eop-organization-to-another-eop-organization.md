...
ms.locfileid: "44617448"
---
# <a name="move-domains-and-settings-from-one-eop-organization-to-another"></a><span data-ttu-id="27ea3-103">Domeinen en instellingen van één EOP-organisatie naar een andere verplaatsen</span><span class="sxs-lookup"><span data-stu-id="27ea3-103">Move domains and settings from one EOP organization to another</span></span>

<span data-ttu-id="27ea3-104">Als u de bedrijfsvereisten wijzigt, moet soms één Microsoft Exchange Online Protection (Tenant) worden opgesplitst in twee afzonderlijke organisaties, twee organisaties worden samengevoegd in één of uw domeinen en EOP-instellingen van de ene organisatie naar de andere worden verplaatst.</span><span class="sxs-lookup"><span data-stu-id="27ea3-104">Changing business requirements can sometimes require splitting one Microsoft Exchange Online Protection (EOP) organization (tenant) into two separate organizations, merging two organizations into one, or moving your domains and EOP settings from one organization to another organization.</span></span> <span data-ttu-id="27ea3-105">De overgang van één EOP-organisatie naar een tweede EOP-organisatie kan een uitdaging zijn, maar met een paar eenvoudige externe Windows PowerShell-scripts en een kleine hoeveelheid voorbereiding kan dit worden bereikt met een relatief klein onderhoudsvenster.</span><span class="sxs-lookup"><span data-stu-id="27ea3-105">Moving from one EOP organization to a second EOP organization can be challenging, but with a few basic remote Windows PowerShell scripts and a small amount of preparation, this can be achieved with a relatively small maintenance window.</span></span>

> [!NOTE]
> <span data-ttu-id="27ea3-106">Instellingen kunnen alleen worden verplaatst van een EOP standalone (Standard) organisatie naar een andere EOP Standard of een Exchange Enterprise CAL met Services (EOP Premium) organisatie, of van een EOP Premium organisatie naar een andere EOP Premium organisatie.</span><span class="sxs-lookup"><span data-stu-id="27ea3-106">Settings can be reliably moved only from an EOP standalone (Standard) organization to either another EOP Standard or an Exchange Enterprise CAL with Services (EOP Premium) organization, or from an EOP Premium organization to another EOP Premium organization.</span></span> <span data-ttu-id="27ea3-107">Omdat sommige premium functies niet worden ondersteund in EOP Standard-organisaties, is de overstap van een EOP Premium-organisatie naar een EOP Standard-organisatie mogelijk niet succesvol.</span><span class="sxs-lookup"><span data-stu-id="27ea3-107">Because some premium features are not supported in EOP Standard organizations, moves from an EOP Premium organization to an EOP Standard organization might not be successful.</span></span> <br><br> <span data-ttu-id="27ea3-108">Deze instructies zijn voor EOP filtering-only organisaties.</span><span class="sxs-lookup"><span data-stu-id="27ea3-108">These instructions are for EOP filtering-only organizations.</span></span> <span data-ttu-id="27ea3-109">Er zijn extra overwegingen bij de overgang van de ene Exchange Online-organisatie naar de andere Exchange Online-organisatie.</span><span class="sxs-lookup"><span data-stu-id="27ea3-109">There are additional considerations in moving from one Exchange Online organization to another Exchange Online organization.</span></span> <span data-ttu-id="27ea3-110">Exchange Online-organisaties vallen buiten het bereik van deze instructies.</span><span class="sxs-lookup"><span data-stu-id="27ea3-110">Exchange Online organizations are out of scope for these instructions.</span></span>

<span data-ttu-id="27ea3-111">In het volgende voorbeeld is Contoso, Ltd. gefuseerd met Contoso Suites.</span><span class="sxs-lookup"><span data-stu-id="27ea3-111">In the following example, Contoso, Ltd. has merged with Contoso Suites.</span></span> <span data-ttu-id="27ea3-112">De volgende afbeelding toont het proces van het verplaatsen van domeinen, e-mailgebruikers en -groepen en instellingen van de bron-EOP-organisatie (contoso.onmicrosoft.com) naar de doel-EOP-organisatie (contososuites.onmicrosoft.com):</span><span class="sxs-lookup"><span data-stu-id="27ea3-112">The following image shows the process of moving domains, mail users and groups, and settings from the source EOP organization (contoso.onmicrosoft.com) to the target EOP organization (contososuites.onmicrosoft.com):</span></span>

![EOP-domeinen en -instellingen verplaatsen](../../media/EOP-Move-domains-and-settings.jpg)

<span data-ttu-id="27ea3-114">De uitdaging bij het verplaatsen van domeinen van de ene organisatie naar de andere is dat een geverifieerd domein niet in twee organisaties tegelijk kan bestaan.</span><span class="sxs-lookup"><span data-stu-id="27ea3-114">The challenge in moving domains from one organization to another is that a verified domain can't exist in two organizations at the same time.</span></span> <span data-ttu-id="27ea3-115">De volgende stappen helpen u om dit te voorkomen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-115">The following steps help you work through this.</span></span>

## <a name="step-1-collect-data-from-the-source-organization"></a><span data-ttu-id="27ea3-116">Stap 1: Gegevens verzamelen van de bronorganisatie</span><span class="sxs-lookup"><span data-stu-id="27ea3-116">Step 1: Collect data from the source organization</span></span>

<span data-ttu-id="27ea3-117">Als u de bronorganisatie opnieuw wilt maken in de doelorganisatie, moet u ervoor zorgen dat u de volgende informatie over de bronorganisatie verzamelt en opslaat:</span><span class="sxs-lookup"><span data-stu-id="27ea3-117">In order to re-create the source organization in the target organization, make sure that you collect and store the following information about the source organization:</span></span>

- <span data-ttu-id="27ea3-118">Domeinen</span><span class="sxs-lookup"><span data-stu-id="27ea3-118">Domains</span></span>

- <span data-ttu-id="27ea3-119">E-mailgebruikers</span><span class="sxs-lookup"><span data-stu-id="27ea3-119">Mail users</span></span>

- <span data-ttu-id="27ea3-120">Groepen</span><span class="sxs-lookup"><span data-stu-id="27ea3-120">Groups</span></span>

- <span data-ttu-id="27ea3-121">Anti-spam</span><span class="sxs-lookup"><span data-stu-id="27ea3-121">Anti-spam</span></span>

  - <span data-ttu-id="27ea3-122">Antispambeleid (ook wel inhoudsfilterbeleid genoemd)</span><span class="sxs-lookup"><span data-stu-id="27ea3-122">Anti-spam policies (also known as content filter policies)</span></span>
  - <span data-ttu-id="27ea3-123">Beleid voor uitgaande spamfilter</span><span class="sxs-lookup"><span data-stu-id="27ea3-123">Outbound spam filter policies</span></span>
  - <span data-ttu-id="27ea3-124">Verbindingsfilterbeleid</span><span class="sxs-lookup"><span data-stu-id="27ea3-124">Connection filter policies</span></span>

- <span data-ttu-id="27ea3-125">Anti-malwarebeleid</span><span class="sxs-lookup"><span data-stu-id="27ea3-125">Anti-malware policies</span></span>

- <span data-ttu-id="27ea3-126">Verbindingslijnen</span><span class="sxs-lookup"><span data-stu-id="27ea3-126">Connectors</span></span>

- <span data-ttu-id="27ea3-127">Regels voor e-mailstroom (ook wel transportregels genoemd)</span><span class="sxs-lookup"><span data-stu-id="27ea3-127">Mail flow rules (also known as transport rules)</span></span>

  > [!NOTE]
  > <span data-ttu-id="27ea3-128">Cmdlet-ondersteuning voor het exporteren en importeren van de e-mailstroomregelverzameling wordt momenteel alleen ondersteund voor EOP Premium-abonnementen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-128">Cmdlet support for the export and import of the mail flow rule collection is currently only supported for EOP Premium subscription plans.</span></span>

<span data-ttu-id="27ea3-129">De eenvoudigste manier om al uw instellingen te verzamelen, is door PowerShell te gebruiken.</span><span class="sxs-lookup"><span data-stu-id="27ea3-129">The easiest way to collect all of your settings is to use PowerShell.</span></span> <span data-ttu-id="27ea3-130">Zie [Verbinding maken met Exchange Online Protection PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-protection-powershell) als u verbinding wilt maken met standalone EOP PowerShell.</span><span class="sxs-lookup"><span data-stu-id="27ea3-130">To connect to standalone EOP PowerShell, see [Connect to Exchange Online Protection PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-protection-powershell).</span></span>

<span data-ttu-id="27ea3-131">Vervolgens u al uw instellingen verzamelen en exporteren naar een .xml-bestand dat moet worden geïmporteerd naar de doel tenant.</span><span class="sxs-lookup"><span data-stu-id="27ea3-131">Next, you can collect all your settings and export them to an .xml file to be imported into the target tenant.</span></span> <span data-ttu-id="27ea3-132">In het algemeen u de uitvoer van de cmdlet **Ophalen** voor elke instelling naar de **cmdlet Export-Clixml** leiden om de instellingen op te slaan in .xml-bestanden, zoals weergegeven in het volgende codevoorbeeld.</span><span class="sxs-lookup"><span data-stu-id="27ea3-132">In general, you can pipe the output of the **Get** cmdlet for each setting to the **Export-Clixml** cmdlet to save the settings in .xml files, as shown in the following code sample.</span></span>

<span data-ttu-id="27ea3-133">Maak in standalone EOP PowerShell een map met de naam Exporteren op een locatie die gemakkelijk te vinden en te wijzigen is in die map.</span><span class="sxs-lookup"><span data-stu-id="27ea3-133">In standalone EOP PowerShell, create a directory called Export in a location that's easy to find and change to that directory.</span></span> <span data-ttu-id="27ea3-134">Bijvoorbeeld:</span><span class="sxs-lookup"><span data-stu-id="27ea3-134">For example:</span></span>
...
<span data-ttu-id="27ea3-135">Het volgende script kan worden gebruikt om alle e-mailgebruikers, groepen, antispaminstellingen, anti-malware-instellingen, connectors en e-mailstroomregels in de bronorganisatie te verzamelen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-135">The following script can be used to collect all the mail users, groups, anti-spam settings, anti-malware settings, connectors, and mail flow rules in the source organization.</span></span> <span data-ttu-id="27ea3-136">Kopieer en plak de volgende tekst in een teksteditor zoals Kladblok, sla het bestand op als Source_EOP_Settings.ps1 in de map Exporteren die u zojuist hebt gemaakt en voer de volgende opdracht uit:</span><span class="sxs-lookup"><span data-stu-id="27ea3-136">Copy and paste the following text into a text editor like Notepad, save the file as Source_EOP_Settings.ps1 in the Export directory you just created, and run the following command:</span></span>
...
<span data-ttu-id="27ea3-137">Voer de volgende opdrachten uit de map Exporteren uit om de .xml-bestanden bij te werken met de doelorganisatie.</span><span class="sxs-lookup"><span data-stu-id="27ea3-137">Run the following commands from the Export directory to update the .xml files with the target organization.</span></span> <span data-ttu-id="27ea3-138">Vervang contoso.onmicrosoft.com en contososuites.onmicrosoft.com door uw bron- en doelorganisatienamen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-138">Replace contoso.onmicrosoft.com and contososuites.onmicrosoft.com with your source and target organization names.</span></span>
...
## <a name="step-2-add-domains-to-the-target-organization"></a><span data-ttu-id="27ea3-139">Stap 2: Domeinen toevoegen aan de doelorganisatie</span><span class="sxs-lookup"><span data-stu-id="27ea3-139">Step 2: Add domains to the target organization</span></span>

<span data-ttu-id="27ea3-140">Voeg domeinen toe aan de doelorganisatie met behulp van het volgende script.</span><span class="sxs-lookup"><span data-stu-id="27ea3-140">Add domains to the target organization by using the following script.</span></span> <span data-ttu-id="27ea3-141">Kopieer en plak de tekst in een teksteditor zoals Kladblok, sla het script op als C:\EOP\Exporteren\Add_Domains.ps1 en voer de volgende opdracht uit:</span><span class="sxs-lookup"><span data-stu-id="27ea3-141">Copy and paste the text into a text editor like Notepad, save the script as C:\EOP\Export\Add_Domains.ps1, and run the following command:</span></span>

```PowerShell
& "C:\EOP\Export\Add_Domains.ps1"
```

<span data-ttu-id="27ea3-142">Deze domeinen worden niet geverifieerd en kunnen niet worden gebruikt om e-mail te routeren, maar nadat de domeinen zijn toegevoegd, u de informatie verzamelen die nodig is om de domeinen te verifiëren en uiteindelijk uw MX-records voor de nieuwe tenant bij te werken.</span><span class="sxs-lookup"><span data-stu-id="27ea3-142">These domains won't be verified and can't be used to route mail, but after the domains are added, you can collect the information needed to verify the domains and eventually update your MX records for the new tenant.</span></span>
...
<span data-ttu-id="27ea3-143">U nu de informatie bekijken en verzamelen van het Microsoft 365-beheercentrum van uw doelorganisatie, zodat u uw domeinen snel verifiëren wanneer de tijd daar is:</span><span class="sxs-lookup"><span data-stu-id="27ea3-143">Now you can review and collect the information from the Microsoft 365 admin center of your target organization so you can quickly verify your domains when the time comes:</span></span>

1. <span data-ttu-id="27ea3-144">Meld u aan bij het Microsoft 365-beheercentrum op <https://portal.office.com> .</span><span class="sxs-lookup"><span data-stu-id="27ea3-144">Sign in to the Microsoft 365 admin center at <https://portal.office.com>.</span></span>

2. <span data-ttu-id="27ea3-145">Klik **op Domeinen**.</span><span class="sxs-lookup"><span data-stu-id="27ea3-145">Click **Domains**.</span></span>

   <span data-ttu-id="27ea3-146">Als u geen domeinen ziet, klikt u op **Navigtion aanpassen,** selecteer **Setup**en klikt u vervolgens op **Opslaan**.</span><span class="sxs-lookup"><span data-stu-id="27ea3-146">If you don't see domains, click **Customize navigtion**, select **Setup**, and then click **Save**.</span></span>

3. <span data-ttu-id="27ea3-147">Klik op elke koppeling **Voor het instellen van start** en ga door de wizard Setup.</span><span class="sxs-lookup"><span data-stu-id="27ea3-147">Click each **Start setup** link, and then proceed through the setup wizard.</span></span>

4. <span data-ttu-id="27ea3-148">Selecteer op de pagina **Eigenaarschap bevestigen** voor **Stapsgewijze instructies voor het uitvoeren van deze stap voor het uitvoeren**van deze stap met Algemene **instructies**.</span><span class="sxs-lookup"><span data-stu-id="27ea3-148">On the **Confirm ownership** page, for **See step-by-step instructions for performing this step with**, select **General instructions**.</span></span>

5. <span data-ttu-id="27ea3-149">Neem de MX-record of TXT-record op die u gebruikt om uw domein te verifiëren en de wizard Setup voltooien.</span><span class="sxs-lookup"><span data-stu-id="27ea3-149">Record the MX record or TXT record that you'll use to verify your domain, and finish the setup wizard.</span></span>

6. <span data-ttu-id="27ea3-150">Voeg de verificatie-TXT-records toe aan uw DNS-records.</span><span class="sxs-lookup"><span data-stu-id="27ea3-150">Add the verification TXT records to your DNS records.</span></span> <span data-ttu-id="27ea3-151">Hiermee u sneller de domeinen in de bronorganisatie verifiëren nadat ze uit de doelorganisatie zijn verwijderd.</span><span class="sxs-lookup"><span data-stu-id="27ea3-151">This will let you more quickly verify the domains in the source organization after they're removed from the target organization.</span></span> <span data-ttu-id="27ea3-152">Zie [DNS-records maken bij een DNS-hostingprovider voor Microsoft 365 voor](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)meer informatie over het configureren van DNS.</span><span class="sxs-lookup"><span data-stu-id="27ea3-152">For more information about configuring DNS, see [Create DNS records at any DNS hosting provider for Microsoft 365](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).</span></span>

## <a name="step-3-force-senders-to-queue-mail"></a><span data-ttu-id="27ea3-153">Stap 3: Afzenders dwingen om e-mail in de wachtrij te plaatsen</span><span class="sxs-lookup"><span data-stu-id="27ea3-153">Step 3: Force senders to queue mail</span></span>

<span data-ttu-id="27ea3-154">Terwijl u uw domeinen van de ene tenant naar de andere verplaatst, moet u de domeinen uit de bronorganisatie verwijderen en deze vervolgens verifiëren in uw doelorganisatie.</span><span class="sxs-lookup"><span data-stu-id="27ea3-154">While moving your domains from one tenant to another, you'll need to delete the domains from the source organization and then verify them in your target organization.</span></span> <span data-ttu-id="27ea3-155">Gedurende deze tijd u e-mail niet door EOP routeren.</span><span class="sxs-lookup"><span data-stu-id="27ea3-155">During this time, you won't be able to route mail through EOP.</span></span>

<span data-ttu-id="27ea3-156">Een optie om afzenders te dwingen e-mail in de wachtrij te plaatsen, is om uw MX-records bij te werken om rechtstreeks naar uw on-premises e-mailserver te wijzen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-156">One option to force senders to queue mail is to update your MX records to point directly to your on-premises mail server.</span></span>

<span data-ttu-id="27ea3-157">Een andere optie is om een ongeldige MX-record in elk domein te plaatsen waar de DNS-records voor uw domein worden bijgehouden (ook wel uw DNS-hostingservice genoemd).</span><span class="sxs-lookup"><span data-stu-id="27ea3-157">Another option is to put an invalid MX record in each domain where the DNS records for your domain are kept (also known as your DNS hosting service).</span></span> <span data-ttu-id="27ea3-158">Hierdoor staat de afzender ervoor dat de afzender uw e-mail in de wachtrij zet en opnieuw probeert (typische pogingen voor het opnieuw proberen zijn 48 uur, maar dit kan per provider verschillen).</span><span class="sxs-lookup"><span data-stu-id="27ea3-158">This will cause the sender to queue your mail and retry (typical retry attempts are for 48 hours, but this might vary from provider to provider).</span></span> <span data-ttu-id="27ea3-159">U invalid.outlook.com gebruiken als ongeldig MX-doel.</span><span class="sxs-lookup"><span data-stu-id="27ea3-159">You can use invalid.outlook.com as an invalid MX target.</span></span> <span data-ttu-id="27ea3-160">Als u de Waarde Tijd om te leven (TTL) verlaagt tot vijf minuten in de MX-record, wordt de wijziging sneller doorgegeven aan DNS-providers.</span><span class="sxs-lookup"><span data-stu-id="27ea3-160">Lowering the Time to Live (TTL) value to five minutes on the MX record will help the change propagate to DNS providers more quickly.</span></span>

<span data-ttu-id="27ea3-161">Zie [DNS-records maken bij een DNS-hostingprovider voor Microsoft 365 voor](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)meer informatie over het configureren van DNS.</span><span class="sxs-lookup"><span data-stu-id="27ea3-161">For more information about configuring DNS, see [Create DNS records at any DNS hosting provider for Microsoft 365](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).</span></span>

> [!IMPORTANT]
> <span data-ttu-id="27ea3-162">Verschillende providers wachtrij e-mail voor verschillende perioden.</span><span class="sxs-lookup"><span data-stu-id="27ea3-162">Different providers queue mail for different periods of time.</span></span> <span data-ttu-id="27ea3-163">U moet uw nieuwe tenant snel instellen en uw DNS-instellingen terugdraaien om te voorkomen dat rapporten (NDR's) niet worden verzonden naar de afzender als de rijtijd verloopt.</span><span class="sxs-lookup"><span data-stu-id="27ea3-163">You'll need to set up your new tenant quickly and revert your DNS settings to avoid non-delivery reports (NDRs) from being sent to the sender if the queuing time expires.</span></span>

## <a name="step-4-remove-users-groups-and-domains-from-the-source-organization"></a><span data-ttu-id="27ea3-164">Stap 4: Gebruikers, groepen en domeinen verwijderen uit de bronorganisatie</span><span class="sxs-lookup"><span data-stu-id="27ea3-164">Step 4: Remove users, groups, and domains from the source organization</span></span>

<span data-ttu-id="27ea3-165">In het volgende script worden gebruikers, groepen en domeinen uit de bron tenant verwijderd met Azure Active Directory PowerShell.</span><span class="sxs-lookup"><span data-stu-id="27ea3-165">The following script removes users, groups, and domains from the source tenant by using Azure Active Directory PowerShell.</span></span> <span data-ttu-id="27ea3-166">Kopieer en plak de volgende tekst in een teksteditor zoals Kladblok, sla het bestand op als C:\EOP\Exporteren\Remove_Users_and_Groups.ps1 en voer de volgende opdracht uit:</span><span class="sxs-lookup"><span data-stu-id="27ea3-166">Copy and paste the following text into a text editor like Notepad, save the file as C:\EOP\Export\Remove_Users_and_Groups.ps1, and run the following command:</span></span>
...
## <a name="step-5-verify-domains-for-the-target-organization"></a><span data-ttu-id="27ea3-167">Stap 5: Domeinen voor de doelorganisatie verifiëren</span><span class="sxs-lookup"><span data-stu-id="27ea3-167">Step 5: Verify domains for the target organization</span></span>

1. <span data-ttu-id="27ea3-168">Meld u aan bij het beheercentrum op [https://portal.office.com](https://portal.office.com) .</span><span class="sxs-lookup"><span data-stu-id="27ea3-168">Sign in to the admin center at [https://portal.office.com](https://portal.office.com).</span></span>

2. <span data-ttu-id="27ea3-169">Klik **op Domeinen**.</span><span class="sxs-lookup"><span data-stu-id="27ea3-169">Click **Domains**.</span></span>

3. <span data-ttu-id="27ea3-170">Klik op elke koppeling **voor het instellen van start** voor het doeldomein en ga door de wizard Setup.</span><span class="sxs-lookup"><span data-stu-id="27ea3-170">Click each **Start setup** link for the target domain and proceed through the setup wizard.</span></span>

## <a name="step-6-add-mail-users-and-groups-to-the-target-organization"></a><span data-ttu-id="27ea3-171">Stap 6: E-mailgebruikers en -groepen toevoegen aan de doelorganisatie</span><span class="sxs-lookup"><span data-stu-id="27ea3-171">Step 6: Add mail users and groups to the target organization</span></span>

<span data-ttu-id="27ea3-172">Een aanbevolen manier voor EOP is om Azure Active Directory te gebruiken om uw on-premises Active Directory te synchroniseren met uw doel tenant.</span><span class="sxs-lookup"><span data-stu-id="27ea3-172">A best practice for EOP is to use Azure Active Directory to sync your on-premises Active Directory to your target tenant.</span></span> <span data-ttu-id="27ea3-173">Zie 'Mapsynchronisatie gebruiken om e-mailgebruikers te beheren' in [EOP beheren](manage-mail-users-in-eop.md)voor meer informatie over hoe u dit doen.</span><span class="sxs-lookup"><span data-stu-id="27ea3-173">For more information about how to do this, see "Use directory synchronization to manage mail users" in [Manage mail users in EOP](manage-mail-users-in-eop.md).</span></span> <span data-ttu-id="27ea3-174">U ook het volgende script gebruiken om uw gebruikers en groepen opnieuw te maken vanaf uw bron tenant.</span><span class="sxs-lookup"><span data-stu-id="27ea3-174">You can also use the following script to recreate your users and groups from your source tenant.</span></span> <span data-ttu-id="27ea3-175">Opmerking: Gebruikerswachtwoorden kunnen niet worden verplaatst.</span><span class="sxs-lookup"><span data-stu-id="27ea3-175">Note: User passwords cannot be moved.</span></span> <span data-ttu-id="27ea3-176">Nieuwe gebruikerswachtwoorden worden gemaakt en opgeslagen in het bestand met de naam UsersAndGroups.ps1.</span><span class="sxs-lookup"><span data-stu-id="27ea3-176">New user passwords are created and saved in the file named UsersAndGroups.ps1.</span></span>

<span data-ttu-id="27ea3-177">Als u het script wilt gebruiken, kopieert en plakt u de volgende tekst in een teksteditor zoals Kladblok, slaat u het bestand op als C:\EOP\Exporteren\Add_Users_and_Groups.ps1 en voert u de volgende opdracht uit:</span><span class="sxs-lookup"><span data-stu-id="27ea3-177">To use the script, copy and paste the following text into a text editor like Notepad, save the file as C:\EOP\Export\Add_Users_and_Groups.ps1, and run the following command:</span></span>
...
## <a name="step-7-add-protection-settings-to-the-target-organization"></a><span data-ttu-id="27ea3-178">Stap 7: Beveiligingsinstellingen toevoegen aan de doelorganisatie</span><span class="sxs-lookup"><span data-stu-id="27ea3-178">Step 7: Add protection settings to the target organization</span></span>

<span data-ttu-id="27ea3-179">U het volgende script uitvoeren vanuit de map Exporteren terwijl u bent aangemeld bij uw doelorganisatie om de instellingen die eerder vanuit de bronorganisatie naar .xml-bestanden zijn geëxporteerd opnieuw te maken.</span><span class="sxs-lookup"><span data-stu-id="27ea3-179">You can run the following script from the Export directory while logged in to your target organization to recreate the settings exported to .xml files earlier from the source organization.</span></span>

<span data-ttu-id="27ea3-180">Kopieer en plak de scripttekst in een teksteditor zoals Kladblok, sla het bestand op als C:\EOP\Exporteren\Import_Settings.ps1 en voer de volgende opdracht uit:</span><span class="sxs-lookup"><span data-stu-id="27ea3-180">Copy and paste the script text into a text editor like Notepad, save the file as C:\EOP\Export\Import_Settings.ps1, and run the following command:</span></span>

```PowerShell
& "C:\EOP\Export\Import_Settings.ps1"
```

<span data-ttu-id="27ea3-181">Dit script importeert de .xml-bestanden en maakt een Windows PowerShell-scriptbestand genaamd Settings.ps1 dat u controleren, bewerken en uitvoeren om uw instellingen voor beveiliging en e-mailstroom opnieuw te maken.</span><span class="sxs-lookup"><span data-stu-id="27ea3-181">This script imports the .xml files and create a Windows PowerShell script file called Settings.ps1 that you can review, edit, and then run to recreate your protection and mail-flow settings.</span></span>
...
## <a name="step-8-revert-your-dns-settings-to-stop-mail-queuing"></a><span data-ttu-id="27ea3-182">Stap 8: Uw DNS-instellingen terugzetten om e-mailwachting te stoppen</span><span class="sxs-lookup"><span data-stu-id="27ea3-182">Step 8: Revert your DNS settings to stop mail queuing</span></span>

<span data-ttu-id="27ea3-183">Als u ervoor kiest om uw MX-records in te stellen op een ongeldig adres, moet de afzenders tijdens uw overgang e-mail in de wachtrij plaatsen, dan moet u deze weer instellen op de juiste waarde zoals opgegeven in het [beheercentrum.](https://admin.microsoft.com)</span><span class="sxs-lookup"><span data-stu-id="27ea3-183">If you chose to set your MX records to an invalid address to cause the senders to queue mail during your transition, you'll need to set them back to the correct value as specified in the [admin center](https://admin.microsoft.com).</span></span> <span data-ttu-id="27ea3-184">Zie [DNS-records maken bij een DNS-hostingprovider voor Microsoft 365 voor](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)meer informatie over het configureren van DNS.</span><span class="sxs-lookup"><span data-stu-id="27ea3-184">For more information about configuring DNS, see [Create DNS records at any DNS hosting provider for Microsoft 365](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).</span></span>
