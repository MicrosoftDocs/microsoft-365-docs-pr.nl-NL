...
| `SenderMailFromAddress` | tekenreeks | Het e-mailadres van de afzender in de kop E-mail FROM, ook wel afzender van de envelop genoemd of het Return-Path mailadres |
...
| `SenderDisplayName` | tekenreeks | De naam van de afzender die wordt weergegeven in het adresboek, meestal een combinatie van een bepaalde of voornaam, een middelste initiale en een achternaam of achternaam. |
...
| `PhishFilterVerdict` | tekenreeks | Door het filteren van e-mail wordt bepaald of het e-mailbericht phish is: Phish of Not Phish |
| `PhishDetectionMethod` | tekenreeks | Methode die wordt gebruikt om de e-mail te detecteren als phish: Malicious URL reputation, Safe Links URL Detonation, Advanced phish filter, General phish filter, Anti-Spoof: Rente-org, Anti-spoof: external domain, Domain impersonation, User impersonation, Brand impersonation |
...
| `ConfidenceLevel` | tekenreeks | Lijst met betrouwbaarheidsniveaus van eventuele spam- of phishingberichten. Voor spam geeft deze kolom het betrouwbaarheidsniveau voor spam weer, waarmee wordt aangegeven of de e-mail is overgeslagen (-1), wordt gevonden dat het geen spam is (0,1), die spam met een gemiddelde betrouwbaarheid (5,6) of als spam met hoge betrouwbaarheid (9) blijkt te zijn. Bij phishing wordt in deze kolom weergegeven of het betrouwbaarheidsniveau 'Hoog' of 'Laag' is. |
| `EmailAction` | tekenreeks | Uiteindelijke actie die wordt ondernomen op basis van filteracties, beleidsregels en gebruikersacties: Bericht verplaatsen naar map Ongewenste e-mail, X-koptekst toevoegen, Onderwerp wijzigen, Bericht omleiden, Bericht verwijderen, Verzenden naar quarantaine, Geen actie ondernomen, BCC-bericht |
| `EmailActionPolicy` | tekenreeks | Actiebeleid dat van kracht is: Antispam zeer vertrouwelijk, Antispam, Antispam-bulkmail, Antispam-phishing, Imitatie van anti-phishingdomein, Imitatie van anti-phishinggebruikers, Anti-phishing spoofing, Anti-phishing graph impersonation, Antimalware, Safe Attachments, Enterprise Transport Rules (ETR) |
...
| `Connectors` | tekenreeks | Aangepaste instructies die de e-mailstroom van de organisatie definiëren en hoe de e-mail is doorgestuurd |
| `OrgLevelAction` | tekenreeks | Actie die wordt ondernomen op het e-mailbericht als antwoord op overeenkomsten met een beleid dat is gedefinieerd op organisatieniveau |
