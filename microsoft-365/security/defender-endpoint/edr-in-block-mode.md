...
3. Zoek in de lijst met resultaten in de **rij AMRunningMode** naar een van de volgende waarden:
   - `Normal` - Defender-service die normaal wordt uitgevoerd. Er zijn geen speciale modi ingeschakeld.
   - `Passive Mode`- Als uw organisatie Microsoft Defender voor Eindpunt gebruikt samen met een niet-Microsoft antivirus-/antimalware-oplossing, wordt Microsoft Defender Antivirus automatisch in de passieve modus. (Realtime-beveiliging en bedreigingen worden niet door de Microsoft Defender Antivirus.)
   - `SxS Passive Mode`- Vergelijkbaar met de passieve modus, maar met de optie om beperkt periodiek scannen in te schakelen.
