...
De aanbevolen strategie van Microsoft voor het optimaliseren van de connectiviteit van externe werknemers is gericht op het snel oplossen van de problemen met de traditionele benadering en het leveren van hoge prestaties met een paar eenvoudige stappen. Met deze stappen past u de oudere VPN-benadering aan voor een klein aantal gedefinieerde eindpunten die gehalhaleleerde VPN-servers omzeilen. Een gelijkwaardig of zelfs superieur beveiligingsmodel kan op verschillende lagen worden toegepast om de noodzaak te verwijderen om al het verkeer aan het begin van het bedrijfsnetwerk te beveiligen. In de meeste gevallen kan dit effectief binnen enkele uren worden bereikt en kan het vervolgens worden schaalbaar voor andere werkbelastingen, zoals de vraag en tijd van de vereisten toestaat.
...
Dit is het meest voorkomende beginscenario voor de meeste zakelijke klanten. Er wordt een gedwongen VPN gebruikt, wat betekent dat 100% van het verkeer naar het bedrijfsnetwerk wordt doorgestuurd, ongeacht het feit dat het eindpunt zich al dan niet binnen het bedrijfsnetwerk bevindt. Extern (internet)afhankelijk verkeer, zoals Office 365 of surfen op internet, wordt vervolgens weer uit de on-premises beveiligingsapparatuur gehaald, zoals proxies. In het huidige klimaat met bijna 100% van de gebruikers die op afstand werken, wordt de VPN-infrastructuur hierdoor zeer belast door dit model en kan dit de prestaties van al het bedrijfsverkeer en dus de onderneming aanzienlijk hinderen om efficiënt te werken in tijden van crisis.
...
| https:// \<tenant\> .sharepoint.com | TCP 443 | Dit is de primaire URL voor SharePoint Online en heeft een hoog bandbreedtegebruik. |
...
De VPN-client moet zo zijn geconfigureerd dat het verkeer naar de **IPs** optimaliseren op deze manier wordt gerouteerd. Hierdoor kan het verkeer gebruikmaken van lokale Microsoft-bronnen, zoals Office 365 Service Front Doors, zoals de [Azure Front Door](https://azure.microsoft.com/blog/azure-front-door-service-is-now-generally-available/) die Office 365-services en connectiviteits-eindpunten zo dicht mogelijk bij uw gebruikers levert. Hierdoor kunnen we zeer hoge prestatieniveaus leveren aan gebruikers waar ze ook ter wereld zijn en profiteren we optimaal van het wereldwijde netwerk van [Microsoft](https://azure.microsoft.com/blog/how-microsoft-builds-its-fast-and-reliable-global-network/)van wereldklasse, dat zeer waarschijnlijk binnen een klein aantal milliseconden van het directe egressie van uw gebruikers valt.
...
In dit artikel vindt u informatie over de manier waarop Teams algemene beveiligingsproblemen beperkt, zoals spraak- of _Session Traversal Utilities for NAT (STUN)_ amplification attacks [.](/openspecs/office_protocols/ms-ice2/69525351-8c68-4864-b8a6-04bfbc87785c)
...
Dus hoewel een gebruiker een TCP/UDP-verbinding kan maken met de bovenstaande gemarkeerde eindpunten optimaliseren, zonder een geldig token om toegang te krijgen tot de tenant in kwestie, kunnen ze zich gewoon niet aanmelden en geen gegevens openen of verplaatsen.
...
Poort 80 wordt alleen gebruikt voor zaken zoals omleiden naar een poort 443-sessie, er worden geen klantgegevens verzonden of zijn toegankelijk via poort 80. [In dit artikel](../compliance/encryption.md) wordt versleuteling beschreven voor gegevens die [](/microsoftteams/microsoft-teams-online-call-flows#types-of-traffic) onderweg zijn en in rust voor Office 365, en in dit artikel wordt beschreven hoe we SRTP gebruiken om Teams-mediaverkeer te beschermen.
...
**Nee,** dat doet het niet. Het werkt alleen in Microsoft Teams-clientversie 1.3.00.13565 of hoger. Deze versie bevat verbeteringen in de manier waarop de client beschikbare netwerkpaden detecteert.
