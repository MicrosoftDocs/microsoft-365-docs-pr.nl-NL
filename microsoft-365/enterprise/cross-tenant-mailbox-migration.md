...
Als een Exchange Online-tenant voorheen postvakken moest verplaatsen naar een andere tenant in dezelfde Exchange Online-service, moesten ze deze volledig buiten gebruik maken van on-premises en vervolgens aan boord brengen naar een nieuwe tenant. Met de nieuwe functie voor migratie van postvakken tussen tenants kunnen tenantbeheerders in zowel bron- als doelten tenants postvakken verplaatsen tussen de tenants met minimale infrastructuurafhankelijkheden in hun on-premises systemen. Hierdoor hoeft u geen postvakken aan boord of aan boord meer te gebruiken.
...
Gebruikers die migreren, moeten aanwezig zijn in het Exchange Online-systeem van de doeltender als MailUsers, gemarkeerd met specifieke kenmerken om de cross-tenantverzet in te stellen. Het systeem wordt niet verplaatst voor gebruikers die niet correct zijn ingesteld in de doel tenant.  
...
Migraties van Exchange-postvakken met verschillende tenants worden alleen ondersteund voor tenants in hybride of cloud, of een combinatie van de twee.
...
Voor de migratiefunctie voor Exchange-postvakken met kruis tenants is autorisatie en scoping vereist voor migraties tussen tenants. Met de Azure Enterprise-toepassing en key vault-opslagoplossingen kunnen tenantbeheerders nu zowel autorisatie als scoping van exchange Online-postvakmigraties van de ene tenant naar de andere beheren. Het verplaatsen van postvakken met meerdere tenants ondersteunt een uitnodigings- en toestemmingsmodel voor het instellen van een Azure Active Directory-toepassing (Azure AD) die wordt gebruikt voor verificatie tussen een tenantpaar. Aanvullende onderdelen, zoals een organisatierelatie en een migratie-eindpunt, zijn ook vereist.
...
U moet ook communiceren met uw vertrouwde partnerbedrijf (met wie u postvakken verplaatst) om hun Microsoft 365-tenant-id te verkrijgen. Deze tenant-id wordt gebruikt in het veld `DomainName` Organisatierelatie.
...
1. Download het SetupCrossTenantRelationshipForTargetTenant.ps1 script voor het instellen van de doel tenant vanuit de [GitHub-opslagplaats.](https://github.com/microsoft/cross-tenant/releases/tag/Preview) 
2. Sla het script (SetupCrossTenantRelationshipForTargetTenant.ps1) op de computer waaruit u het script gaat uitvoeren.
3. Maak een externe PowerShell-verbinding met de doel tenant van Exchange Online. Zorg er nogmaals voor dat u de benodigde machtigingen hebt om de implementatiescripts uit te voeren om de opslag en het certificaat van de Azure Key Vault, De toepassing Postvak verplaatsen, EXO-migratie-eindpunt en de EXO-organisatierelatie te configureren.
...
6. Het script wordt onderbroken en u wordt gevraagd of u akkoord wilt gaan met de migratietoepassing voor Exchange-postvakken die tijdens dit proces is gemaakt. Hier volgt een voorbeeld.
...
4. Download het SetupCrossTenantRelationshipForResourceTenant.ps1 script voor de installatie van de bron tenant hier vanuit de GitHub-opslagplaats: [https://github.com/microsoft/cross-tenant/releases/tag/Preview](https://github.com/microsoft/cross-tenant/releases/tag/Preview) . 
...
Als u fouten ontvangt tijdens de configuratie van de bron- of doelten tenants, kunt u het script VerifySetup.ps1 [GitHub](https://github.com/microsoft/cross-tenant/releases/tag/Preview) uitvoeren en de uitvoer controleren.
...
1. Voor postvakken die vanuit een bronorganisatie worden verplaatst, moet u een MailUser-object inrichten in de doelorganisatie: 
   ...
   - Er kunnen al extra kenmerken worden opgenomen in hybride terugschrijven van Exchange. Zo niet, dan moeten ze worden opgenomen. 
...
4. Gebruikers in de doelorganisatie moeten een licentie hebben met de juiste Exchange Online-abonnementen die van toepassing zijn op de organisatie. U kunt een licentie toepassen op voorhand van een postvakverhuis, maar alleen wanneer de doelmailuser correct is ingesteld met ExchangeGUID- en proxyadressen. Het toepassen van een licentie voordat de ExchangeGUID wordt toegepast, resulteert in een nieuw postvak dat is ingericht in de doelorganisatie. 

    > [!Note]
    > Wanneer u een licentie op een postvak- of MailUser-object toe te passen, worden alle SMTP-type proxyAddresses geschuurd om ervoor te zorgen dat alleen geverifieerde domeinen worden opgenomen in de matrix Exchange EmailAddresses. 

5. U moet ervoor zorgen dat de doelmailuser geen eerdere ExchangeGuid heeft die niet overeenkomen met de Bron ExchangeGuid. Dit kan gebeuren als de doel-meu eerder een licentie voor Exchange Online had en een postvak had ingericht. Als de doelmailuser eerder een licentie had voor of een ExchangeGuid had die niet overeen komt met de Source ExchangeGuid, moet u een opschoning van de cloud-meu uitvoeren. Voor deze cloud-MEUs's kunt u `Set-User <identity> -PermanentlyClearPreviousMailboxInfo` uitvoeren.  
...
Migraties van Exchange-postvakken met verschillende tenants worden verzonden als migratiebatchs die zijn gestart vanuit de doelten tenant. Dit is vergelijkbaar met de manier waarop on-boarding migratiebatchs werken bij het migreren van Exchange on-premises naar Microsoft 365. 
...
Nee, de inhoud van de Teams-chatmap wordt niet gemigreerd tussen tenants.  
...
```
**Hoe hebben we toegang tot Outlook op dag 1 nadat het postvak voor gebruik is verplaatst?**
...
**Van welke Exchange-RBAC-rollen moet ik lid zijn om een cross-tenantverzet in te stellen of te voltooien?**
...
Begin met het uitvoeren van VerifySetup.ps1 script [op GitHub](https://github.com/microsoft/cross-tenant/releases/tag/Preview) en bekijk de uitvoer.
...
- **Probleem: MailUser-objecten met 'externe' primaire SMTP-adressen worden gewijzigd /opnieuw ingesteld op 'interne' geclaimde domeinen van het bedrijf**
   ...
   | Naam                                              |
   |---------------------------------------------------|
   | Geavanceerde eDiscovery-opslag (500 GB)               |
   | Klanten-lockbox                                  |
   ...
   | Exchange Online Archivering voor Exchange Online     |
   | Exchange Online Archivering voor Exchange Server     |
   | Invoegvoeggebruik in Exchange Online Inactieve gebruiker              |
