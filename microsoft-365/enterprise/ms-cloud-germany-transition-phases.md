...
|Stap(en)|Duur|Verantwoordelijke partij|Omschrijving|
|---|---|---|---|
...
Dit zorgt ervoor dat verificatieaanvragen voor Exchange beschikbaarheidsaanvragen van gebruikers in de migratietoestand die zich richten op de hybride on-premises omgeving, worden geverifieerd om toegang te krijgen tot de on-premises service. Op dezelfde manier zorgt dit ervoor dat aanvragen worden verificatie van on-premises naar Office 365 globale services-eindpunten.
Nadat Azure AD-migratie (fase 2) is voltooid, moet de beheerder van de on-premises Exchange-topologie (hybride) een nieuw eindpunt voor verificatieservice toevoegen voor de Office 365 Globale services.

Met deze opdracht van Exchange PowerShell vervangt u de tenant-id van uw organisatie in de `<TenantID>` Azure-portal op Azure Active Directory.
...
Aanvullende informatie voor Cloud Solution Providers is beschikbaar in [partner tenantmigratie.](ms-cloud-germany-transition-add-csp.md#partner-tenant-migration)
...
Door klanten beheerde DNS-instellingen voor Autodiscover die momenteel verwijzen naar Microsoft Cloud Deutschland, moeten worden bijgewerkt om te verwijzen naar het Office 365 Global-eindpunt na voltooiing van de Exchange Online-fase (fase 5).

Bestaande DNS-vermeldingen met CNAME die naar autodiscover-outlook.office.de moeten worden bijgewerkt om te wijzen **naar autodiscover.outlook.com.**
...
- Uitvoeren , een PowerShell-cmdlet, tijdens de migratie van Exchange van Microsoft Cloud Deutschland naar Office 365 services werken `Test-MigrationServerAvailabiilty` mogelijk niet. De migratie werkt echter correct nadat de migratie is voltooid.
- Als clients problemen hebben met referenties of autorisatie nadat postvakken zijn gemigreerd, kunt u de on-premises beheerdersreferenties in het migratie-eindpunt opnieuw instellen door deze uit te voeren of door hetzelfde in te stellen met `Set-MigrationEndpoint -Identity <endpointName> -Credential $(Get-Credential)` Exchange Control Panel (ECP).
...
**Van toepassing op:** Alle klanten die Exchange Online
...
Zorg ervoor dat u bekend bent met het [prework voor uw Installatieprocedure voor Microsoft Dynamics 365.](ms-cloud-germany-transition-add-pre-work.md#dynamics-365)
