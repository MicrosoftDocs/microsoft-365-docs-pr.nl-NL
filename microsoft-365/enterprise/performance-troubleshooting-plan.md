...
- Ent_O365
description: In dit artikel kunt u problemen met de prestaties van Office 365 oplossen en zelfs enkele van de meest voorkomende problemen oplossen.
...
Wilt u weten welke stappen u moet ondernemen om vertragingen, vastlopen en trage prestaties tussen SharePoint Online, OneDrive voor Bedrijven, Exchange Online of Skype voor Bedrijven Online en uw clientcomputer te identificeren en op te lossen? Voordat u ondersteuning belt, kan dit artikel u helpen bij het oplossen van prestatieproblemen met Office 365 en kunt u zelfs enkele van de meest voorkomende problemen oplossen.
...
Als u nieuw bent in netwerkprestaties en een plan voor de lange termijn wilt maken om de prestaties tussen uw clientapparaten en Office 365 te controleren, bekijkt u [Office 365](performance-tuning-using-baselines-and-history.md)performance tuning en probleemoplossing - Beheerder en IT Pro.
...
- Hulpprogramma's installeren voor het verzamelen en opnemen van informatie:
  ...
  - Gebruik een schermrecorder of voer de stappenrecorder (PSR.exe) uit die bij Windows Vista en hoger wordt geleverd, om een record bij te houden van de stappen die u tijdens het testen ondernomen hebt.
...
- Optioneel: Als u Exchange Online test, kunt u het hulpprogramma Exchange Client Performance Analyzer uitvoeren vanaf de Office 365-beheerconsole.
- Reproduceer de exacte stappen die het prestatieprobleem veroorzaken.
- Stop de trace van uw Netmon of een ander hulpmiddel.
- Voer op de opdrachtregel een traceroute uit naar uw Office 365-abonnement door de volgende opdracht te typen en vervolgens op Enter te drukken:
...
1. Open de opdrachtprompt **(cmd starten** \> **of** \> **cmd** **met Windows-toets).** \> 
...
Met het hulpprogramma Netwerkcontrole[(Netmon)](https://www.microsoft.com/download/details.aspx?id=4865)van Microsoft worden pakketten geanalyseerd, dat wil zeggen verkeer, die tussen computers op netwerken worden gebruikt. Door Netmon te gebruiken om verkeer met Office 365 te traceren, kunt u pakketkoppen vastleggen, bekijken en lezen, tussenliggende apparaten identificeren, belangrijke instellingen controleren op netwerkhardware, zoek naar pakketjes en de verkeersstroom volgen tussen computers in uw bedrijfsnetwerk en Office 365. Omdat de werkelijke hoeveelheid verkeer is versleuteld, dat wil zeggen dat het (reist via poort 443 via SSL/TLS, kunt u de verzonden bestanden niet lezen. In plaats daarvan krijgt u een ongefilterd spoor van het pad dat door het pakket wordt gevolgd, zodat u het probleemgedrag kunt opsporen.
...
![HTTPWatch met het tabblad Netwerk voor een paginabelasting van de startpagina van Office 365.](../media/021a2c64-d581-49fd-adf4-4c364f589d75.PNG)
...
1. Gebruik **Start** Run typePSR.exeOK of klik \>  \> op het type  \>  **Windows-PSR.exe** \> en druk vervolgens op  \> Enter.
...
Netmon-traces kunnen veel verkeer bevatten. Als u geen ervaring hebt met het lezen van de trace, is het waarschijnlijk dat u de trace de eerste keer zult openen. Het eerste wat u moet doen, is het signaal scheiden van het achtergrondgeluid in de trace. U hebt getest op Office 365 en dat is het verkeer dat u wilt zien. Als u gewend bent om door traceringen te navigeren, hebt u deze lijst mogelijk niet nodig.

Verkeer tussen uw client en Office 365 gaat via TLS, wat betekent dat de hoofdruimte van het verkeer wordt versleuteld en niet leesbaar is in een algemene Netmon-trace. Uw prestatieanalyse hoeft niet de details van de gegevens in het pakket te weten. Het is echter zeer geïnteresseerd in pakketkoppen en de informatie die ze bevatten.
...
Als u Office 365 Outlook Online als voorbeeld neemt, begint het verkeer ongeveer als dit:

- DNS Standard Query en DNS Response voor outlook.office365.com met overeenkomende Query-query's. Het is belangrijk om de tijds verschuiving voor deze omslag te noteren, evenals waar in de wereld de globale DNS van Office 365 de aanvraag voor naamresolutie verzendt. Ideaal, zo lokaal mogelijk, in plaats van halverwege de wereld.

- Een HTTP GET-aanvraag waarvan het statusrapport permanent is verplaatst (301)

- RWS-verkeer, inclusief RWS Connect-aanvragen en Connect-antwoorden. (Dit is Remote Winsock die een verbinding voor u maakt.)
...
Alle delen van het verkeer zijn belangrijk en verbonden, maar kleine gedeelten van de trace bevatten informatie die met name belangrijk is voor het oplossen van prestatieproblemen. Daarom richten we ons op deze gebieden. Aangezien we genoeg problemen met de prestaties van Office 365 bij Microsoft hebben uitgevoerd om een lijst met veelvoorkomende problemen samen te stellen, richten we ons op deze problemen en op het gebruik van de hulpprogramma's die we nodig hebben om ze vervolgens uit te zoeken.
...
Als u de waarde van Windows Scaling wilt zien die door uw netwerkverbinding wordt gebruikt, vouwt u eerst de SYN uit en vervolgens de bijbehorende SYN/ACK.
...
### <a name="tcp-idle-time-settings"></a>TCP Idle Time Settings

Historisch gezien zijn de meeste perimeternetwerken geconfigureerd voor tijdelijke verbindingen, wat betekent dat niet-actieve verbindingen over het algemeen worden beëindigd. Inactieve TCP-sessies kunnen worden beëindigd met proxies en firewalls van meer dan 100 tot 300 seconden. Dit is problematisch voor Outlook Online omdat er lange termijnverbindingen worden gemaakt en gebruikt, ongeacht of ze inactief zijn of niet.

Wanneer verbindingen worden beëindigd door proxy- of firewallapparaten, is de client niet op de hoogte en een poging om Outlook Online te gebruiken betekent dat een clientcomputer herhaaldelijk probeert de verbinding nieuw leven in te blazen voordat u een nieuwe maakt. Mogelijk ziet u vasthangingen in het product, aanwijzingen of trage prestaties bij het laden van pagina's.
...
Bekijk in Netmon het veld Time Offset voor een retourneer. Een retourreis is de tijd tussen het verzenden van een aanvraag door de client naar de server en het ontvangen van een antwoord. Controleer tussen de client en het uitgangspunt (bijvoorbeeld. Client -- \> Proxy) of de client naar Office 365 (Client -- \> Office 365). U kunt dit in veel soorten pakketten zien.
...
Er zijn bandbreedteberekeningen voor Office 365 beschikbaar op deze pagina Netwerkplanning en prestatieafstemming [voor Office 365.](network-planning-and-performance.md)
...
Als u latentie in een trace wilt bijhouden, hebt u baat bij het hebben van het IP-adres van de clientcomputer en het IP-adres van de DNS-server in Office 365. Dit is voor het eenvoudiger filteren van trace. Als u verbinding maakt via een proxy, hebt u het IP-adres van uw clientcomputer, het PROXY/egress IP-adres en het DNS-IP-adres van Office 365 nodig om het werk gemakkelijker te maken.
...
> [!NOTE]
> PsPing verzendt geen ICMP-pakketten. Het pingt met TCP-pakketten via een specifieke poort, zodat u alle TCP-pakketten kunt gebruiken die u kent om geopend te zijn. Probeer in Office 365, dat gebruikmaakt van SSL/TLS, poort :443 aan uw PsPing te koppelen.
...
Als u de traag presterende Office 365-pagina hebt geladen terwijl u een netwerkspoor doet, moet u een Netmon- of Wireshark-trace filteren op `DNS` . Dit is een van de IPs die we zoeken.
...
6. Open een opdrachtprompt (Cmd type Start Run of Windows Key type cmd) en wijzig de adreslijst in de adreslijst waar u PsPing hebt geïnstalleerd om de \> \> opdracht \> PsPing uit te voeren. In mijn voorbeelden ziet u dat ik een 'Perf'-map heb gemaakt op de hoofdmap van C. U kunt hetzelfde doen voor snelle toegang.
7. Typ de opdracht zodat u psPing maakt op basis van het IP-adres van de DNS-server van Office 365 vanaf uw eerdere Netmon-trace, inclusief het poortnummer, zoals `psping -n 20 132.245.24.82:445` . Hiermee krijgt u een steekproef van 20 pings en wordt de latentie gemiddeld wanneer PsPing stopt.

Als u via een proxyserver naar Office 365 gaat, zijn de stappen iets anders. U psping eerst naar uw proxyserver om een gemiddelde latentiewaarde in milliseconden naar proxy/egress en back te krijgen en vervolgens PsPing uit te voeren op de proxy of op een computer met een directe internetverbinding om de ontbrekende waarde te krijgen (de waarde naar Office 365 en terug).

Als u PsPing wilt uitvoeren vanaf de proxy, hebt u twee millisecondenwaarden: Clientcomputer naar proxyserver of uitgangspunt en proxyserver naar Office 365. En u bent klaar. Hoe dan ook, waarden opnemen.
...
Als proxyverificatie is aangemaakt, moet u elke keer dat u een nieuwe TCP-verbinding met Office 365 maakt om informatie te krijgen, achter de schermen een verificatieproces doorlopen. Wanneer u bijvoorbeeld overschakelt van Agenda naar E-mail in Outlook Online, wordt dit geverifieerd. En in SharePoint Online, als op een pagina media of gegevens van meerdere sites of locaties worden weergegeven, verifieert u voor elke verschillende TCP-verbinding die nodig is om de gegevens weer te geven.
...
![Een Netmon-trace van De belasting van Outlook Online gefilterd op DNS en het gebruik van Gesprekken zoeken en VERVOLGENS DNS om de resultaten te beperken.](../media/763cf20e-7b48-4a37-9449-c9978cfe118b.PNG)
...
Services zoals Outlook Online in Office 365 verlenen clients meerdere lange termijnverbindingen. Daarom kan elke gebruiker meer verbindingen gebruiken die een langere levensduur vereisen.
...
Het doel is om een MSS van 1460 bytes te zien voor het verzenden van gegevens. Als u achter een proxy zit of als u een NAT gebruikt, vergeet dan niet om deze test uit te voeren van client naar proxy/egress/NAT en van proxy/egress/NAT naar Office 365 voor de beste resultaten! Dit zijn verschillende TCP-sessies.
...
Als u achter een proxy zit of als u een NAT gebruikt, vergeet dan niet om deze test uit te voeren van client naar proxy/egress/NAT en van proxy/egress/NAT naar Office 365 voor de beste resultaten! Dit zijn verschillende TCP-sessies.
...
Waar in de wereld Office 365 probeert uw DNS-oproep op te lossen, heeft dit gevolgen voor de verbindingssnelheid.

Nadat de eerste DNS-zoekactie in Outlook Online is voltooid, wordt de locatie van die DNS gebruikt om verbinding te maken met het dichtstbijzijnde datacenter. U wordt verbonden met een CAS-server van Outlook Online, die het backbone-netwerk gebruikt om verbinding te maken met het datacenter (dC) waar uw gegevens zijn opgeslagen. Dit gaat sneller.

Wanneer u SharePoint Online gebruikt, wordt een gebruiker die naar het buitenland reist, doorgestuurd naar zijn of haar actieve datacenter. Dit is de dC waarvan de locatie is gebaseerd op de thuisbasis van de SPO-tenant (dus een dC in de Verenigde Staten als de gebruiker in de VS is gevestigd).
...
Verzoeken om naamsresolutie van de DNS-servers van de client naar de DNS-servers van Microsoft moeten in de meeste gevallen leiden tot het retourneren van het IP-adres van een regionaal datacenter (dC). Wat betekent dit voor u? Als uw hoofdkantoor zich in Bangalore, India, maar u reist in de Verenigde Staten, wanneer uw browser een aanvraag voor Outlook Online indient, moeten de DNS-servers van Microsoft u IP-adressen geven aan datacenters in de Verenigde Staten, een regionaal datacenter. Als e-mail nodig is vanuit Outlook, worden deze gegevens verspreid over het snelle backbonenetwerk van Microsoft tussen de datacenters.
...
### <a name="office-365-application-troubleshooting"></a>Probleemoplossing voor Office 365-toepassingen
