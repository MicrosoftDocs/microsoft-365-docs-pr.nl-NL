...
|4.  <br/> |In de weergave **Bestanden** wordt het aantal totaal- en actieve bestanden weergegeven. Een bestand wordt als actief beschouwd als het binnen een bepaalde periode is opgeslagen, gesynchroniseerd, gewijzigd of gedeeld.  <br/> OPMERKING: Een bestandsactiviteit kan meerdere keren optreden voor één bestand, maar telt slechts als één actief bestand. Zo kunt u bijvoorbeeld hetzelfde bestand meerdere malen opslaan en synchroniseren in een bepaalde periode, maar dit wordt als één actief bestand en één gesynchroniseerd bestand meegeteld in de gegevens.           |
|5.  <br/> |De weergave **Opslag** toont de trend voor de hoeveelheid OneDrive-opslagruimte die u gebruikt. Zie Controleren of een gebruiker de [standaardopslaglimiet of een specifieke limiet heeft](https://docs.microsoft.com/onedrive/set-default-storage-space#check-if-a-user-has-the-default-storage-limit-or-a-specific-limit)als u de opslaglimieten wilt controleren.  <br/> OPMERKING: De grootte bevat alle versies en metagegevens die aan de bestanden zijn gekoppeld.           |
