...
<a name="BKMK_find_your_dns_1"></a> Ga naar de pagina met de DNS-records voor uw domein. Als u werkt in Windows Server 2008, gaat u naar **Start**  >  **Run**. Als u in een Windows Server 2012 werkt, drukt u op Windows en **r**. Typ **dnsmgmnt.msc** en selecteer **OK.** Vouw in DNS Manager **\<DNS server name\> \> opzoekzones doorsturen uit.** Selecteer uw domein. Nu kunt u de DNS-records gaan maken.
...
- Kopieer in de rij MX in Exchange Online sectie van de pagina DNS-records toevoegen in Microsoft de waarde die wordt vermeld onder Adres van punten. U gebruikt deze waarde in de record die u in deze taak maakt. 
- Ga op de pagina DNS Manager voor het domein naar **Action**  >  **Mail Exchanger (MX)**. Zie Uw [DNS-records](#find-your-dns-records-in-windows-based-dns)zoeken in Windows dns om deze pagina voor het domein te vinden.  
...
Voordat u de DNS-records toevoegt om uw Microsoft-services, moet Microsoft bevestigen dat u de eigenaar bent van het domein dat u toevoegt. Hiertoe voegt u een record toe aan de hand van onderstaande stappen.
