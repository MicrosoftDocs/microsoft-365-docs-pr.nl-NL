...
Wanneer u heeft besloten om [retentielabels](retention.md) te gebruiken om documenten en e-mailberichten in Microsoft 365 te bewaren of te verwijderen, weet u mogelijk dat u vele, mogelijk honderden, retentielabels moet aanmaken en publiceren. De aanbevolen methode om retentielabels op grote schaal aan te maken is door [bestandsplan](file-plan-manager.md) te gebruiken in het Microsoft 365-compliancecentrum. U kunt echter ook [PowerShell](retention.md#powershell-cmdlets-for-retention-policies-and-retention-labels) gebruiken.
  
Gebruik de informatie, sjabloonbestanden en voorbeelden en het script in dit artikel om bulksgewijs retentielabels te maken en deze te publiceren in beleid voor retentielabels. Vervolgens kunnen de retentielabels worden [ toegepast door beheerders en gebruikers](create-apply-retention-labels.md#how-to-apply-published-retention-labels).

De opgegeven instructies bieden geen ondersteuning voor retentielabels die automatisch worden toegepast.

Overzicht: 

1. Maak in Excel een lijst met uw retentielabels aan en een lijst met het hun beleidsregels voor retentielabels.

2. Gebruik PowerShell om de retentielabels en het beleid voor retentielabels in die lijsten aan te maken.
  
## <a name="disclaimer"></a>Disclaimer

De voorbeeldscripts in dit artikel worden niet ondersteund door een standaardondersteuningsprogramma of -service van Microsoft. De voorbeeldscripts worden zonder enige garantie ONGEWIJZIGD verstrekt. Microsoft wijst alle impliciete garanties met inbegrip van, maar niet beperkt tot, impliciete garanties van verkoopbaarheid en geschiktheid voor een bepaald doel af. Het risico dat ontstaat als gevolg van het gebruik of prestaties van de voorbeeldscripts ligt geheel bij u. In geen geval kan Microsoft, haar auteurs of anderen die bij het maken, produceren of leveren van de scripts zijn betrokken, aansprakelijk worden gehouden voor enige schade om welke reden dan ook (met inbegrip van maar niet beperkt tot schade door verlies van bedrijfswinsten, belemmering van de bedrijfsuitvoering, verlies van bedrijfsinformatie of andere geldelijke verliezen) voortvloeiend uit het gebruik of de onmogelijkheid van het gebruik van de voorbeeldscripts, zelfs als Microsoft op de hoogte is gesteld van de mogelijkheid van dergelijke schade.
  
## <a name="step-1-create-a-csv-file-for-the-retention-labels"></a>Stap 1: een .csv-bestand maken voor de retentielabels

1. Kopieer het volgende .csv-voorbeeldbestand voor een sjabloon en voorbeeldgegevens voor vier verschillende retentielabels en plak deze in Excel. 

2. Converteer de tekst naar kolommen: tabblad **Gegevens** \> **Tekst naar kolommen** \> **Met scheidingstekens** \> **Komma** \> **Algemeen**

2. Vervang de voorbeelden door vermeldingen voor uw eigen retentielabels en instellingen. Zie [New-ComplianceTag](/powershell/module/exchange/new-compliancetag) voor meer informatie over de parameterwaarden.

3. Sla het werkblad op als een .csv-bestand op een locatie die u gemakkelijk kunt vinden voor een latere stap. Bijvoorbeeld: C:\>Scripts\Labels.csv
...
### <a name="sample-csv-file-for-retention-labels"></a>Voorbeeld van een .csv-bestand voor retentielabels
...
## <a name="step-2-create-a-csv-file-for-the-retention-label-policies"></a>Stap 2: een .csv-bestand maken voor het beleid voor retentielabels

1. Kopieer het volgende .csv-voorbeeldbestand voor een sjabloon en voorbeeldgegevens voor drie verschillende beleidsregels voor retentielabels en plak deze in Excel. 

2. Converteer de tekst naar kolommen: tabblad **Gegevens** \> **Tekst naar kolommen** \> **Met scheidingstekens** \> **Komma** \> **Algemeen**

2. Vervang de voorbeelden door vermeldingen voor uw eigen beleid voor retentielabels en instellingen. Zie [Nieuw-RetentionCompliancePolicy](/powershell/module/exchange/new-retentioncompliancepolicy) voor meer informatie over de parameterwaarden voor deze cmdlet.

3. Sla het werkblad op als een .csv-bestand op een locatie die u gemakkelijk kunt vinden voor een latere stap. Bijvoorbeeld:`<path>Policies.csv`
...
### <a name="sample-csv-file-for-retention-policies"></a>Voorbeeld van een .csv-bestand voor retentiebeleidsregels
...
## <a name="step-3-create-the-powershell-script"></a>Stap 3: het PowerShell-script aanmaken

1. Kopieer en plak het volgende PowerShell-script in Kladblok.

2. Sla het bestand op met de bestandsextensie **.ps1** op een locatie die u gemakkelijk kunt terugvinden. Bijvoorbeeld:`<path>CreateRetentionSchedule.ps1`

Opmerkingen:

- In het script wordt u gevraagd de twee bronbestanden op te geven die u in de vorige twee stappen hebt gemaakt:
    - Als u het bronbestand niet opgeeft om de retentielabels te maken, gaat het script verder met het aanmaken van beleidsregels voor retentielabels. 
    - Als u het bronbestand niet opgeeft om het beleid voor retentielabels te maken, gaat het script verder met het aanmaken van retentielabels.

- Het script genereert een logboekbestand met de acties die worden ondernomen en of de actie is geslaagd of mislukt. Zie de laatste stap voor instructies om dit logboekbestand te vinden.
...
## <a name="step-4-run-the-powershell-script"></a>Stap 4: het PowerShell-script uitvoeren

Maak als eerste [verbinding met PowerShell van het beveiligings- en compliancecentrum](/powershell/exchange/connect-to-scc-powershell).

Voer vervolgens het script uit waardoor de retentielabels worden gemaakt en gepubliceerd:
  
1. Voer in de PowerShell-sessie van het Beveiligings- en compliancecentrum het pad in, gevolgd door de tekens `.\` en de bestandsnaam van het script, en druk vervolgens op ENTER om het script uit te voeren. Bijvoorbeeld:
    
    ```powershell
    <path>.\CreateRetentionSchedule.ps1
    ```

2. In het script wordt u gevraagd om de locatie van de .csv-bestanden die u in de vorige twee stappen hebt gemaakt op te geven. Voer het pad in, gevolgd door de tekens `.\` en de bestandsnaam van het .csv-bestand, en druk op ENTER. Bijvoorbeeld bij de eerste prompt:
    
    ```powershell
    <path>.\Labels.csv
    ```

## <a name="step-5-view-the-log-file-with-the-results"></a>Stap 5: het logboekbestand met de resultaten weergeven

Gebruik het logboekbestand dat het script heeft gemaakt om de resultaten te controleren en na te gaan of er fouten zijn die moeten worden opgelost.

U vindt het logboekbestand op de volgende locatie, hoewel de cijfers in de voorbeeldbestandsnaam afwijken.
